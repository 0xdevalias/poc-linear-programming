...
# TODO: I'm not sure if this version is actually optimising very well at the moment..? It might be broken..
...
# Minimum usage percentage required to justify purchasing a new bottle
min_usage_percentage = 0.5  # 50%
...
for supp in supplements:
  ...
  units_needed_from_bottles = s * daily_dose - current_stock            # Units needed from purchased bottles
  total_units_available = current_stock + (x[label] * bottle_size)      # Total units from current stock + all purchased bottles
  min_units_to_use = min_usage_percentage * bottle_size * x[label]      # Minimum units to use to justify buying a new bottle

  # Constraint: The required units must not exceed the total available from the purchased bottles and current stock.
  problem += units_needed_from_bottles <= (x[label] * bottle_size)

  # Constraint: Calculate the leftover units (r[label]) as the difference between available and required units.
  problem += r[label] == total_units_available - (s * daily_dose)

  # Constraint: Ensure that the percentage of the bottle used is above the minimum usage threshold.
  problem += units_needed_from_bottles >= min_units_to_use
